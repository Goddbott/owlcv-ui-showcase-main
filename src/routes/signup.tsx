...
export const Route = createFileRoute("/signup")({
  head: () => ({ meta: [{ title: "Sign Up — OwlCV" }, { name: "description", content: "Create your free OwlCV account." }] }),
  component: SignUp,
});
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-primary-soft/40 via-background to-amber-soft/30 px-4 py-12">
      <div className="w-full max-w-md card-soft p-8 shadow-card">
        <div className="text-center">
          ...
          <h1 className="mt-6 text-2xl font-extrabold">Create your free account</h1>
          <p className="mt-2 text-sm text-muted-foreground">Start building beautiful resumes in minutes</p>
        </div>

        {success ? (
          <div className="mt-8 rounded-lg bg-emerald-50 p-4 text-emerald-800 ring-1 ring-emerald-200">
            <p className="font-bold">Check your email!</p>
            <p className="mt-1 text-sm">We've sent a confirmation link to {email}. Please click it to verify your account.</p>
          </div>
        ) : (
          <form className="mt-8 space-y-4" onSubmit={handleSignUp}>
            ...
            <Field label="Full Name" type="text" placeholder="Sarah Johnson" value={fullName} onChange={(e) => setFullName(e.target.value)} required />
...
function Field({ label, type, placeholder, value, onChange, required }: { label: string; type: string; placeholder: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; required?: boolean }) {
  return (
    <div>
      ...
      <input type={type} className="input-base" placeholder={placeholder} value={value} onChange={onChange} required={required} />
    </div>
  );
}
